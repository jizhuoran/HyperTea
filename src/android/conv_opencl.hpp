...
__kernel void deconv1_col2Im(global Dtype* col,
                     global Dtype* im,
                     const int im_offset) {
    ...
    int im_ch  = gid / (256 * 256);
    int im_pix = gid % (256 * 256);
    int im_h   = (im_pix / 256) + 1;
    int im_w   = (im_pix % 256) + 1;

    int start_h = (im_h < 1 * (4 - 1) + 1)
                      ? 0
                      : (im_h - (1 * (4 - 1) + 1)) / 2 + 1;
    int end_h   = min(128, im_h / 2 + 1);
    int start_w = (im_w < 1 * (4 - 1) + 1)
                      ? 0
                      : (im_w - (1 * (4 - 1) + 1)) / 2 + 1;
    int end_w = min(128, im_w / 2 + 1);

    int ch_offset = im_ch * 128 * 128 * 4 * 4;
    col += ch_offset;
    ...
    for(int cy = start_h; cy < end_h; cy++)
    {
        ...
        {
            if((im_h - cy * 2) % 1 == 0 && (im_w - cx * 2) % 1 == 0)
            {
                int col_off_y = cy + (((im_h - cy * 2) / 1) * 4 * 128);
                int col_off_x = cx + (((im_w - cx * 2) / 1) * 128 * 128);

                tmp += (Dtype)(col[col_off_y * 128 + col_off_x]);
            }
        }
    }
    im_off[gid] = tmp;
...
__kernel void deconv2_col2Im(global Dtype* col,
                     global Dtype* im,
                     const int im_offset) {
    ...
    int im_ch  = gid / (512 * 512);
    int im_pix = gid % (512 * 512);
    int im_h   = (im_pix / 512) + 1;
    int im_w   = (im_pix % 512) + 1;

    int start_h = (im_h < 1 * (4 - 1) + 1)
                      ? 0
                      : (im_h - (1 * (4 - 1) + 1)) / 2 + 1;
    int end_h   = min(256, im_h / 2 + 1);
    int start_w = (im_w < 1 * (4 - 1) + 1)
                      ? 0
                      : (im_w - (1 * (4 - 1) + 1)) / 2 + 1;
    int end_w = min(256, im_w / 2 + 1);

    int ch_offset = im_ch * 256 * 256 * 4 * 4;
    col += ch_offset;
    ...
    for(int cy = start_h; cy < end_h; cy++)
    {
        for(int cx = start_w; cx < end_w; cx++)
        {
            if((im_h - cy * 2) % 1 == 0 && (im_w - cx * 2) % 1 == 0)
            {
                int col_off_y = cy + (((im_h - cy * 2) / 1) * 4 * 256);
                int col_off_x = cx + (((im_w - cx * 2) / 1) * 256 * 256);

                tmp += (Dtype)(col[col_off_y * 256 + col_off_x]);
            }
        }
    }
    im_off[gid] = tmp;
...
__kernel void deconv3_col2Im(global Dtype* col,
                     global Dtype* im,
                     const int im_offset) {
    ...
    int im_ch  = gid / (512 * 512);
    int im_pix = gid % (512 * 512);
    int im_h   = (im_pix / 512) + 4;
    int im_w   = (im_pix % 512) + 4;

    int start_h = (im_h < 1 * (9 - 1) + 1)
                      ? 0
                      : (im_h - (1 * (9 - 1) + 1)) / 1 + 1;
    int end_h   = min(512, im_h / 1 + 1);
    int start_w = (im_w < 1 * (9 - 1) + 1)
                      ? 0
                      : (im_w - (1 * (9 - 1) + 1)) / 1 + 1;
    int end_w = min(512, im_w / 1 + 1);

    int ch_offset = im_ch * 512 * 512 * 9 * 9;
    col += ch_offset;
    ...
    for(int cy = start_h; cy < end_h; cy++)
    {
        for(int cx = start_w; cx < end_w; cx++)
        {
            if((im_h - cy * 1) % 1 == 0 && (im_w - cx * 1) % 1 == 0)
            {
                int col_off_y = cy + (((im_h - cy * 1) / 1) * 9 * 512);
                int col_off_x = cx + (((im_w - cx * 1) / 1) * 512 * 512);

                tmp += (Dtype)(col[col_off_y * 512 + col_off_x]);
            }
        }
    }
    im_off[gid] = tmp;
